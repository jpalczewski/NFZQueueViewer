...
env = Environment(variables=vars, CPPPATH=['/home/erxyi/EPiPHANY/lib'])
...
if env['r'] == 'l':
    ...
    if platform.system() == "Linux":
        ...
        os.system('python2 build_web/manage.py runfcgi daemonize=false method=threaded host=' + WEB_SRV_HOST + ' port=' + WEB_SRV_PORT)
    ...
    os.system(BROWSER_CMD)
    os.system('python2 build_web/manage.py runserver ' + WEB_CLIENT_HOST + ':' + WEB_CLIENT_PORT)
elif env['t'] == 'w':
    if(platform.system() == "Linux"):
        os.system('python2 build_web/manage.py test version current calcpy')
    elif(platform.system() == "Windows"):
        os.system('python2 build_web\manage.py test version current calcpy')
elif env['t'] == 'j':
    ...
    SConscript(['calc2/SConscript', 'web/SConscript', 'client/SConscript'], exports=['env'] )
